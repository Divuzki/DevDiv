// DevDiv Since 2020
[].forEach.call(
  document.getElementById("div_id_hashtag").getElementsByTagName("div"),
  function (el) {
    ...
    let hiddenInput = document.createElement("input"),
      ...
      tags = [],
      hashtags = document.getElementById("id_hashtags"),
      div_id_hashtags = document.getElementById("div_id_hashtags");

    div_id_hashtags.classList.add("none");

    hashtags.setAttribute("type", "hidden");
    hashtags.setAttribute("name", `${el.getAttribute("data-name")}s`);
    ...
    function refreshTags() {
      ...
      hashtags.value = tagsList.join(",").replaceAll("#", "");
      ...
      if (hiddenInput.value.charAt(0) == "#" && hiddenInput.value.length > 2) {
        ...
        if (hiddenInput.value == "" || null) {
          ...
          alert(
            "This is a hashtag, You need to add the symbol '#' in " +
              hiddenInput.value +
              ""
          );
        }
...
$("#id_form").on("submit", function (e) {
  ...
  if (
    $("#id_hashtag").val().charAt(0) == "#" &&
    $("#id_hashtag").val().length >= 2
  ) {
    $.ajax({
      method: "POST",
      url: "/hashtag/",
